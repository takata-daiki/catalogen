...
/**
 * Helpless copy the code, because they can not inherit XlsTable, and XlsTable
 * can not read from the PL/Sql developer to export excel data Major rewrite of
 * the (@ link # getValue (int, String)) method. Also increases the counting
 * method of handling scientific
 * 
 * @author lengda
 * @since 2009-12-7 ä¸ĺ10:28:01
 */
public class XlsTable extends AbstractTable {
	...
	static ITableMetaData createMetaData(String tableName, HSSFRow sampleRow,
		...
		logger.debug("createMetaData(tableName={}, sampleRow={}) - start",
				tableName, sampleRow);
		...
		for (int i = 0;; i++) {
			...
			if (columnName != null) {
				...
				// Bugfix for issue ID 2818981 - if a cell has a formatting but
				// no
				// name also ignore it
			...
			byte underline = cell.getCellStyle().getFont(workbook)
					.getUnderline();
			if (underline == 1) {
	...
	private BigDecimal toBigDecimal(double cellValue) {
		...
		// Treatment for scientific notation (for accurate to five decimal
		// places)
	...
	public Object getValue(int row, String column) throws DataSetException {
		...
		case HSSFCell.CELL_TYPE_FORMULA:
			throw new DataTypeException("Formula not supported at row=" + row
					+ ", column=" + column);
