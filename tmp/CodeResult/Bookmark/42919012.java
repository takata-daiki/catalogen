...
package com.softmedia.remote.presentation;

import com.softmedia.remote.R;
import com.softmedia.remote.application.GlobalApp;
import com.softmedia.remote.application.Utils;
import com.softmedia.remote.domain.BookmarkBase;
import com.softmedia.remote.domain.ConnectionReference;
import com.softmedia.remote.domain.ManualBookmark;
import com.softmedia.remote.services.BookmarkBaseGateway;
...
public class BookmarkActivity extends PreferenceActivity implements OnSharedPreferenceChangeListener
{
	...
	public void onCreate(Bundle savedInstanceState)
	{
		...
		Utils.initializeCheckLicenseOrAd(this);
	...
	private void initBookmarkSettings(SharedPreferences sharedPreferences)
	{
		...
		bookmarkSettingsChanged(sharedPreferences, "bookmark.enable_clipboard");
		bookmarkSettingsChanged(sharedPreferences, "bookmark.audio_redirect");
		bookmarkSettingsChanged(sharedPreferences, "bookmark.enable_drive_redirect");
		bookmarkSettingsChanged(sharedPreferences, "bookmark.drive_redirect_path");
	}
	...
	{	
		...
		else if (key.equals("bookmark.audio_redirect"))
		{
			ListPreference listPreference = (ListPreference)findPreference(key);
			listPreference.setSummary(listPreference.getEntry());			
		}
		else if (key.equals("bookmark.enable_drive_redirect"))
		{
			boolean enabled = sharedPreferences.getBoolean(key, false);
			findPreference("bookmark.drive_redirect_path").setEnabled(enabled);			
		}
		else if (key.equals("bookmark.drive_redirect_path"))
		{
			String path = sharedPreferences.getString("bookmark.drive_redirect_path", Environment.getExternalStorageDirectory().getAbsolutePath());
			findPreference("bookmark.drive_redirect_path").setSummary(path);
		}
	}
