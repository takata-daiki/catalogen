...
/**
 * This is Jessie's implementation of a {@link javax.net.ssl.SSLContext} engine,
 * and is available under the algorithm names ``SSLv3'', ``SSL'', ``TLSv1'', and
 * ``TLS''.
 */
public final class Context extends SSLContextSpi {
	...
	public Context() {
		String codec = Util
				.getSecurityProperty("jessie.clientSessionContext.codec");
		String codecClass = null;
		if (codec == null) {
			codec = "null";
		}
		if (codec.equalsIgnoreCase("xml")) {
			codecClass = "org.metastatic.jessie.provider.XMLSessionContext";
		} else if (codec.equalsIgnoreCase("jdbc")) {
			codecClass = "org.metastatic.jessie.provider.JDBCSessionContext";
		} else if (codec.equalsIgnoreCase("null")) {
			codecClass = "org.metastatic.jessie.provider.SessionContext";
		} else {
		...
		try {
			...
			clientSessions = (SessionContext) cl.loadClass(codecClass)
					.newInstance();
		} catch (Exception ex) {
			ex.printStackTrace();
		...
		try {
			...
			serverSessions = (SessionContext) cl.loadClass(codecClass)
					.newInstance();
		} catch (Exception ex) {
			ex.printStackTrace();
			throw new IllegalArgumentException(ex.toString());
	...
	protected javax.net.ssl.SSLServerSocketFactory engineGetServerSocketFactory() {
		if (keyManager == null
				|| (trustManager == null && srpTrustManager == null)
				|| random == null) {
			throw new IllegalStateException();
		}
		return new SSLServerSocketFactory(trustManager, srpTrustManager,
				keyManager, random, serverSessions);
	}
	...
	protected javax.net.ssl.SSLSocketFactory engineGetSocketFactory() {
		...
		return new SSLSocketFactory(trustManager, keyManager, random,
				clientSessions);
	}
	...
	private X509TrustManager defaultTrustManager()
			throws KeyManagementException {
		try {
			TrustManagerFactory fact = TrustManagerFactory.getInstance(
					"JessieX509", "Jessie");
