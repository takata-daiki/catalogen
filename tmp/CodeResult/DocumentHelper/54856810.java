 ...
 */
public class TestSAXReaderImpl implements SAXReader{

    public static TestSAXReaderImpl getInstance() {
        return _instance;
    }
    ...
    protected org.dom4j.io.SAXReader getSAXReader(boolean validate) {
        ...
        if (!GetterUtil.getBoolean(PropsUtil.get(PropsKeys.XML_VALIDATION_ENABLED))) {
            validate = false;
        }
    ...
    private static Log _log = LogFactoryUtil.getLog(TestSAXReaderImpl.class);

    private static TestSAXReaderImpl _instance = new TestSAXReaderImpl();
}
