...
public final class DocumentHelper {
	...
	public static IRegion[] findWicketRegions(final IEditorPart editor, final IDocument document, final int offset) {
		...
		try {
			...
			if (wicketIdRegion == null) {
				try {
					final IRegion li = document.getLineInformationOfOffset(offset);
					for (int i = li.getOffset() + li.getLength(); i > li.getOffset(); i--) {
						if (document.getChar(i) == '"') {
							// search again, but from the end of line
							wicketIdRegion = findStringArgumentInJava(document, i - 1);
							break;
						}
					}
					// really no wicket:id found, so nothing to do here
					if (wicketIdRegion == null) {
						return null;
					}
				} catch (BadLocationException e) {
			...
			for (final IJavaElement javaElement : javaElements) {
				...
				if (STRING_RESOURCE_MODEL.equals(javaElement.getElementName()) || RESOURCE_MODEL.equals(javaElement.getElementName()) || (GET_STRING.equals(javaElement.getElementName()) && isWicketComponent)) {
	 ...
	 * @param offset where the search should begin
