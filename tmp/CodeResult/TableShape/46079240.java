...
public class QueryBuilderEditor extends JPanel {
	...
	public UID getDataSourceEntity() {
		return parent.getDataSourceEntity();
	}
	...
	private void serializeParameters(Element el) {
		...
		for (DatasourceParameterVO paramvo : (model.getParameters())) {
			...
			entry.addAttribute("mandatory", paramvo.getMandatory() == false ? "false" : "true");
			if (paramvo.getValueListProvider() != null) {
				try {
					...
					vlp.addAttribute("name", paramvo.getValueListProvider().getName());
					vlp.addAttribute("type", paramvo.getValueListProvider().getType() == null ? null : paramvo.getValueListProvider().getType().getType());
					vlp.addAttribute("value", paramvo.getValueListProvider().getValue());
	...
	protected void serializeHeader(Element el) {
		...
		if (parent.getEntity() != null) {
			el.addAttribute("entity", parent.getEntity().getStringifiedDefinitionWithEntity(E.ENTITY));
		}
		if (parent.getParentEntity() != null) {
			el.addAttribute("parentEntity", parent.getParentEntity().getStringifiedDefinitionWithEntity(E.ENTITY));
		}
		if (parent.isWithRuleClass()) {
			el.addAttribute("withRuleClass", Boolean.toString(parent.isWithRuleClass()));
		}
	 ...
	 */
	public List<UID> getUsedDatasources() {
		final List<UID> lstResult = new ArrayList<UID>();
		final DefaultComponentModel model = (DefaultComponentModel) viewer.getModel();
		for (Iterator<AbstractShapeModel.Layer> iterLayer = model.getVisibleLayers().iterator(); iterLayer.hasNext();) {
			final AbstractShapeModel.Layer layer = iterLayer.next();
			...
			for (Iterator<Shape> iterShapes = layer.getShapes().iterator(); iterShapes.hasNext();) {
				...
				if (shape instanceof TableShape) {
					...
					if (tableshape.getTable().isQuery()) {
						lstResult.add(tableshape.getTable().getDatasourceVO().getId());
					}
				}
			}
		}
	 ...
	 * @throws NuclosBusinessException
	 */
	protected void serializeColumns(Element el) throws CommonParseException {
		...
		while (enumeration.hasMoreElements()) {
			...
			if (entry.getTable() == null && entry.getColumn() != null) {
				throw new CommonParseException("Die Spalte " + entry.getColumn().getName() + " enth\u00e4lt keinen Tabellennamen.");
			}
			if (entry.getTable() != null && entry.getColumn() != null) {
	...
	class XMLContentHandler implements ContentHandler, LexicalHandler {
        ...
        public void startElement(String namespaceURI, String localName, String qName, Attributes atts) {
			if (qName.equals(QueryBuilderConstants.TAG_HEADER)) {
				...
				if (atts.getValue("parentEntity") != null) {
					parent.setParentEntity(atts.getValue("parentEntity"));
				}
				if (atts.getValue("withRuleClass") != null) {
					parent.setWithRuleClass(Boolean.parseBoolean(atts.getValue("withRuleClass")));
				}
			}
			...
			// process parameters
				...
				final String sMandatory = atts.getValue("mandatory");
				pnlTableSelection.getParameterPanel().addParameter(null, sName, sType, sDescription, sMandatory == null ? false : Boolean.parseBoolean(sMandatory));
			}
			// process sql
