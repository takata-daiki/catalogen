...
public class TableShape extends ComponentAdapter implements Comparable<TableShape> {
	...
	private static final Logger LOG = Logger.getLogger(TableShape.class);
	...
	protected void initColumns() {
		for (Column column : table.getColumns()) {
			...
			columnMap.put(constraintcolumn.getName().toUpperCase(), constraintcolumn);
			columns.add(constraintcolumn);
		}

		for (Constraint constraint : table.getConstraints()) {
			for (Column column : constraint.getColumns()) {
				final ConstraintColumn cc = columnMap.get(column.getName().toUpperCase());
				if (cc != null) {
    ...
    public int compareTo(TableShape shape) {
		return StringUtils.compareIgnoreCase(table.toString(), shape.getTable().toString());
	}
