    ...
    implements VueConstants, XMLUnmarshalListener
{
    ...
    */
    ...
    public static final boolean COLLAPSE_IS_GLOBAL = false;
...
//                 }
    ...
    public void associateOntologicalType (OntType t) {
    	VueMetadataElement vme = new VueMetadataElement();
        vme.setObject(t);
        getMetadataList().getMetadata().add(vme);
        
        for(OntType type : t.getSuperTypes())
        	associateOntologicalType(type);
    }
...
//             e.printStackTrace();
  ...
  /*  public String getStyledLabel()
    ...
    public float getTangentAngleImpl(float x, float y) {
    	return 0.0f;
    }
    
    // UCLA CNP - get angle (wrt x-axis) of the tangent line to the border at x and y 
    public float getTangentAngle(float x, float y) {
    	return getTangentAngleImpl(x, y);
    }
...
//     }
    ...
    public void setHidden(HideCause cause, boolean hide) {
        if (hide)
        {
        	//for(LWComponent d : getAllDescendents())
        	//{
        		//d.setHidden(cause);
        	//}
            setHidden(cause);
    ...
    public boolean isVisible() {
        return mHideBits == 0 && !isAncestorCollapsed();
    }
