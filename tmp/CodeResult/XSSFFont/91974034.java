...
public class ExcelBetreuerinnenAU {
	...
	public void erstelleExcel(ArrayList<String> spalten, String suchevon, String suchebis) {
		try {
			...
			for (String spaltenname : spalten) {
				if (!spaltenname.equals("Aufgehört")) {
			...
			headerCell00.setCellValue("Aufgehört");
			...
			ArrayList<Betreuerin> sortierte_liste = sort
					.sortBetreuerin("Aufgehört", betreuerinnen, suchevon, suchebis);
			...
			for (int i = 0; i < sortierte_liste.size(); i++) {
				...
				for (int j = 0; j < spalten.size(); j++) {
					...
					if (spalten.get(j).equals("Angefangen")) {
						cell.setCellValue(new XSSFRichTextString(b.getAngefangen()));
					}
				}
			}
