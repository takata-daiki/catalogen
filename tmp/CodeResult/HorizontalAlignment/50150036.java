 ...
 ******************************************************************************/
package at.tuwien.prip.annotator.editors.annotator;
...
import at.tuwien.prip.annotator.LearnUIPlugin;
...
import at.tuwien.prip.annotator.preferences.PreferenceConstants;
import at.tuwien.prip.annotator.utils.BrowserMonitor;
import at.tuwien.prip.annotator.utils.DocWrapConstants;
import at.tuwien.prip.annotator.utils.DocumentGraphFactory;
import at.tuwien.prip.annotator.utils.PDFUtils;
import at.tuwien.prip.annotator.utils.benchmark.DiademBenchmarkEngine;
import at.tuwien.prip.annotator.views.DocumentGraphCanvas;
import at.tuwien.prip.annotator.views.IPlotMouseListener;
...
IModelChangedListener 
{
	...
	private Group group2, group3, group4, adjGroup;
	 ...
	 * Constructor.
	 */
	public DocWrapEditor(final WrapperEditor we) {
		this.we = we;
	 ...
	 * @param input
	 */
	public DocWrapEditor(WrapperEditor we, IEditorSite site, IEditorInput input)
	{
		this(we);
		try 
		{
...
//		adjGroup.setLayoutData(rowData);
	...
	private ImageDescriptor getImageDescriptor(String relativePath) {
		...
		try {
			...
			Bundle bundle = Platform.getBundle("at.tuwien.dbai.annotator");
...
//				}
	...
	public void documentUpdated(DocumentUpdateEvent ev) {
		...
		{	
			if (ev.getDocumentUpdate().getType() == UpdateType.DOCUMENT_CHANGED)
			{
				//redraw
				canvas.plot(true);
				canvas.calibrateGraphPosition();
	...
	public void selectionChanged(SelectionChangedEvent event) {
		if (event.getSource() instanceof PDFViewerSWT) {
			StructuredSelection ssel = (StructuredSelection) event
			.getSelection();
			org.eclipse.swt.graphics.Rectangle rect = (org.eclipse.swt.graphics.Rectangle) ssel
			.getFirstElement();

			// PDFViewerSWT viewer = (PDFViewerSWT) event.getSource();
			// double zoom = viewer.getZoomLevel();
			...
			.getDocumentSubgraphUnderRectangle(base, r, false);

//			canvas.setFocusRectangle(r);
			canvas.calibrateGraphPosition();
	...
	public void setWrapperEditor(WrapperEditor we) {
		this.we = we;
	}
	...
	PaintListener, IPlotMouseListener, MouseWheelListener {
		...
		public void mouseUp(MouseEvent e) 
		{
			if (e.button == 1) {
				...
				if (focusRectangle.width > 0 && focusRectangle.height > 0) {
					...
					ISegmentGraph sub = DocGraphUtils
					.getDocumentSubgraphUnderRectangle(dg, rect, false);
		...
		public void mouseMove(MouseEvent e) {
			...
			int y = new Integer((int) ((e.y - canvas.getyTrans()) / scale));
			...
			int maxY = (int) (bounds.x + bounds.height);
			y -= (int)canvas.yOff/scale;
			y = maxY - y;
			
			xCoordValue.setText(x+"");
			yCoordValue.setText(y+"");
	...
	public void blinkRectangle (Rectangle rect) 
	{
		this.canvas.setHighlightRectangle(rect);
		canvas.calibrateGraphPosition();
	}
