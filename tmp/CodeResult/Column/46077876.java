...
public class Column implements Serializable {
	...
	public String toString() {
		if (toString == null || !LangUtils.equal(table, toStringTable)) {
			toString = name;
			toStringTable = table;
			String sFieldName = getFieldNameIfAny();
			if (sFieldName != null) {
				toString = sFieldName;
			}
		}
	...
	public String getFieldNameIfAny() {
		...
		if (table != null) {
			try {
				EntityMeta<?> eMeta = SpringApplicationContextHolder.getBean(IMetaProvider.class).getByTablename(table.getName());
				if (StringUtils.compareIgnoreCase(SF.VERSION.getDbColumn(), name) == 0) {
					result = SF.VERSION.getFieldName();
				} else 
				if (StringUtils.compareIgnoreCase(SF.PK_ID.getDbColumn(), name) == 0) {
					result = "<" + SF.PK_ID.getFieldName() + ">";
				} else 
				if (StringUtils.compareIgnoreCase(SF.PK_UID.getDbColumn(), name) == 0) {
					result = "<" + SF.PK_UID.getFieldName() + ">";
				} else 
				if (StringUtils.compareIgnoreCase(SF.IMPORTVERSION.getDbColumn(), name) == 0) {
					result = SF.IMPORTVERSION.getFieldName();
				} else 
				if (StringUtils.compareIgnoreCase(SF.ORIGINUID.getDbColumn(), name) == 0) {
					result = SF.ORIGINUID.getFieldName();
				}
				String thisColForEqualitiy = FieldMetaUtils.getColumnForEquality(name);
				for (FieldMeta<?> fMeta : eMeta.getFields()) {
					String thatColForEqualitiy = FieldMetaUtils.getColumnForEquality(fMeta.getDbColumn());
					if (thisColForEqualitiy.equals(thatColForEqualitiy)) {
						result = fMeta.getFieldName();
						break;
					}
				}
			} catch (Exception ex) {
