...
public class EntityFieldGroupNucletContentGenerator extends AbstractNucletContentGenerator {
	...
	public EntityFieldGroupNucletContentGenerator(NucletGenerator generator) {
		super(generator, E.ENTITYFIELDGROUP);
	}
		...
		for (Row row : sheet) {
			...
			for (int i = 0; i < COLUMN_COUNT; i++) {
				...
				try {
					switch (i) {
					case COL_NAME:
						storeField(E.ENTITYFIELDGROUP.name.getUID(), getStringValue(cell));
	...
	public UID getIdByName(String name) throws NuclosBusinessException {
		
		for (EntityObjectVO<UID> eo : getResult()) {
			if (LangUtils.equals(eo.getFieldValue(E.ENTITYFIELDGROUP.name.getUID()), name)) {
