...
*******************************************************************************/
	...
	//  Write Parameters To Summary Output File
		String summaryFileName = Parameters.outputDirectory + "/" + Parameters.expID + "_summary.txt";
		FileWriter summaryOutput = new FileWriter(summaryFileName);
		parmValues.outputParameters(summaryOutput);
	//	Write best-of-run fitness matrix to bestOfRunFitnessOutput

	//	Write average population fitness matrix to avgPopFitnessOutput
		String avgPopFitnessFileName = Parameters.outputDirectory + "/" + Parameters.expID + "_avgpopfitness";
		FileWriter avgPopFitnessOutput = new FileWriter(avgPopFitnessFileName);

	//	Write best-of-gen fitness matrix to bestOfGenFitnessOutput
		String bestOfGenFitnessFilename = Parameters.outputDirectory + "/" + Parameters.expID + "_bestofgenfitness";
		FileWriter bestOfGenFitnessOutput = new FileWriter(bestOfGenFitnessFilename);

	//	Write first-optimum-gen matrix to firstOptimumGenOutput
		String firstOptimumGenFilename = Parameters.outputDirectory + "/" + Parameters.expID + "_firstoptimumgen";
		FileWriter firstOptimumGenOutput = new FileWriter(firstOptimumGenFilename);

	//	Write best-of-run fitness matrix to bestOfRunFitnessOutput
		String bestOfRunFitnessFilename = Parameters.outputDirectory + "/" + Parameters.expID + "_bestofrunfitness";
		FileWriter bestOfRunFitnessOutput = new FileWriter(bestOfRunFitnessFilename);
	...
	//	an else_if block below to instantiate the problem.
		...
		else if (Parameters.problemType.equals("CF")) {
				problem = new CF();
		}
	...
	//	Initialize RNG, array sizes and other objects
		...
		bestOfGenChromo = Chromo.makeChromo();
		bestOfRunChromo = Chromo.makeChromo();
		bestOverAllChromo = Chromo.makeChromo();

		if (Parameters.minORmax.equals("max")){
			defaultBest = 0;
			defaultWorst = 99999999999999999999999999.0;
		}
		else{
			defaultBest = 99999999999999999999999999.0;
			defaultWorst = 0;
		}
		...
		for (R = 1; R <= Parameters.numRuns; R++){
			...
			for (int i=0; i<Parameters.popSize; i++){
				member[i] = Chromo.makeChromo();
				child[i] = Chromo.makeChromo();
			}

			int firstOptimumGen = -1;
			...
			for (G=0; G<Parameters.generations; G++){
				...
				for (int i=0; i<Parameters.popSize; i++){
					...
					if (Parameters.minORmax.equals("max")){
						if (member[i].rawFitness > bestOfGenChromo.rawFitness){
							member[i].copyB2A(bestOfGenChromo, member[i]);
							bestOfGenR = R;
							bestOfGenG = G;
						}
						if (member[i].rawFitness > bestOfRunChromo.rawFitness){
							member[i].copyB2A(bestOfRunChromo, member[i]);
							bestOfRunR = R;
							bestOfRunG = G;
						}
						if (member[i].rawFitness > bestOverAllChromo.rawFitness){
							member[i].copyB2A(bestOverAllChromo, member[i]);
							bestOverAllR = R;
							bestOverAllG = G;
						}
					}
				...
				if (firstOptimumGen == -1 &&
				   (int) bestOfGenChromo.rawFitness == (int) (Parameters.numGenes * Parameters.geneSize)) {
					firstOptimumGen = G;
				}
				...
				Hwrite.right(averageRawFitness, 25, 3, summaryOutput);
				Hwrite.right(stdevRawFitness, 25, 3, summaryOutput);
				summaryOutput.write("\n");


				avgPopFitnessOutput.write(averageRawFitness + " ");
				bestOfGenFitnessOutput.write(bestOfGenChromo.rawFitness + " ");
		...
		// *********************************************************************
				...
				int parent2 = -1;
				int startAt = 0;
				if(Parameters.enableEliteSelection == 1)
				{
					startAt = 2;
				}
				//  Assumes always two offspring per mating
				for (int i=startAt; i<Parameters.popSize; i=i+2)
				{

					//	Select Two Parents
					parent1 = member[0].selectParent();  //used to be static call to Chromo
					parent2 = parent1;
					while (parent2 == parent1){
						parent2 = member[0].selectParent(); //used to be static call to Chromo
					}

					//	Crossover Two Parents to Create Two Children
					randnum = r.nextDouble();
					if (randnum < Parameters.xoverRate){
						member[0].mateParents(parent1, parent2, member[parent1], member[parent2], child[i], child[i+1]); //used to be static call to Chromo
					}
					else {
						member[0].mateParents(parent1, member[parent1], child[i]); //used to be static call to Chromo
						member[0].mateParents(parent2, member[parent2], child[i+1]); //used to be static call to Chromo
					}
				} // End Crossover
				
				//hack for elite selection
				if(Parameters.enableEliteSelection == 1)
				{
					member[0].copyB2A(child[0],bestOfGenChromo);
					member[0].copyB2A(child[1],bestOfGenChromo);
					startAt = 1;
				}
			    
				//	Mutate Children
				for (int i=startAt; i<Parameters.popSize; i++){
					child[i].doMutation();
				}

				//	Swap Children with Last Generation
				for (int i=0; i<Parameters.popSize; i++){
					member[0].copyB2A(member[i], child[i]); //used to be static call to Chromo
				}
			...
			avgPopFitnessOutput.write("\n");
			bestOfGenFitnessOutput.write("\n");
			bestOfRunFitnessOutput.write(bestOfRunChromo.rawFitness + "\n");
			firstOptimumGenOutput.write(firstOptimumGen + "\n");
		...
		summaryOutput.write("\n");
		summaryOutput.close();
		avgPopFitnessOutput.close();
		bestOfGenFitnessOutput.close();
		bestOfRunFitnessOutput.close();
		firstOptimumGenOutput.close();

		Stats avgPopFitnessStats = new Stats(avgPopFitnessFileName);
		avgPopFitnessStats.writeStats();

		Stats bestOfGenFitnessStats = new Stats(bestOfGenFitnessFilename);
		bestOfGenFitnessStats.writeStats();

		Stats bestOfRunFitnessStats = new Stats(bestOfRunFitnessFilename);
		bestOfRunFitnessStats.writeStats();

		Stats firstOptimumGenStats = new Stats(firstOptimumGenFilename);
		firstOptimumGenStats.writeStats();
