...
public class ConverterToPatternShape {
	...
	if (shapeView instanceof GroupShapeView) {
	    ...
	    patternShape.setIncrement(BlockShape.X, Number.ZERO);
	    patternShape.setIncrement(BlockShape.Y, Number.ZERO);
	    ...
	    patternShape.setShape(groupShape);
	    IncrementAttributeHandle<Number> deltaXHandle = PatternShape.getIncrementHandle(PatternShape.X, true);
	    patternShape.addAttribute(deltaXHandle, 
		                      patternView.getDeltaXExpression().getExpressionValueSource());
	    IncrementAttributeHandle<Number> deltaYHandle = PatternShape.getIncrementHandle(PatternShape.Y, true);
...
//	shape.removeAttributeChangeListener(shapeListener);
	...
	if (canvas != null && !(shapeView instanceof PatternView)) {
	    ...
	    blockShape.moveTo(new Number(x), new Number(y));
	}
