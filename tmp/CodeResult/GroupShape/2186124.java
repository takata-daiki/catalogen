...
public class ExpresserServiceImpl extends RemoteServiceServlet implements ExpresserService {
	...
	// 0: userKey
	// 1: XML of the previous model
	// 2: application version number
	// 3: project names as a string separated by commas
	// 4: an initial supply of UUID strings separated by commas
	...
	if (userKey == null) {
		...
		dao.getAllColorSpecificRuleExpressions(projectKey);
	...
	if (projectNameObject != null) {
		...
		if (oldProjectName.equalsIgnoreCase(projectName)) {
		    // already know this project name
	...
	if (projectName == null) {
	    // no risk of a name collision since users can't name projects with spaces
	...
	// a buffer of 1000 GUIDs -- whenever one is used a new one is supplied
	// need a fairly large buffer in case importing a model without GUIDs
	result[4] = guids;
	return result;
    }
    ...
    public String tileCreatedOrUpdated(
	    int x, int y, String colorName, String tileId, String userKey, String projectName, String xmppRecipients) {
	if (tileId == null) {
	...
	if (projectName == null || projectName.equals(UNNAMED_PROJECT)) {
	    ...
	    // convert to upper case so project names are case insensitive
	...
	// could check that it is "owned" by userKey
	DAO dao = ServerUtils.getDao();
    ...
    public String patternCreatedOrUpdated(
	    String id, String buildingBlockId, int modelX, int modelY, String xml, String userKey, String projectName) {
	DAO dao = ServerUtils.getDao();
	if (id == null) {
...
//        }
	...
	// and [1] is any warnings or error messages to be sent tot he client
