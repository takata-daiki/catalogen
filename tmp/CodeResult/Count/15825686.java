...
public class HibernateEntityDao implements EntityDao {
	 ...
	 */
	public <T> List<T> search(org.beangle.dao.query.Query<T> query) {
		if (query instanceof LimitQuery) {
			org.beangle.dao.query.LimitQuery<T> limitQuery = (org.beangle.dao.query.LimitQuery<T>) query;
	...
	@SuppressWarnings("unchecked")
	public List<T> searchNamedQuery(String queryName, Object... params) {
		Query query = getSession().getNamedQuery(queryName);
		return QuerySupport.setParameter(query, params).list();
	}
	...
	public static final class QuerySupport {
		...
		private static Query buildHibernateQuery(org.beangle.dao.query.Query<?> bquery,
				final Session hibernateSession) {
			Query hibernateQuery = null;
			if (bquery.getLang().equals(org.beangle.dao.query.Lang.HQL)) {
		 ...
		 * @return
		 */
		public static int count(final org.beangle.dao.query.LimitQuery<?> limitQuery,
				final Session hibernateSession) {
			final org.beangle.dao.query.Query<?> cntQuery = limitQuery.getCountQuery();
			if (null == cntQuery) {
		 ...
		 */
		@SuppressWarnings("unchecked")
		public static <T> List<T> find(final org.beangle.dao.query.Query<T> query,
				final Session hibernateSession) {
			if (query instanceof LimitQuery<?>) {
