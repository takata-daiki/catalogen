...
import com.thimbleware.jmemcached.storage.multibloomfilter.MultiBloomFilterCacheStorage;
...
import com.thimbleware.jmemcached.storage.bloomfilter.BloomFilterStorageCacheStorage;
...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
        ...
        int expected_total_element = 1000;
        int hash_num = 3;  
       /*
        if(args.length >0)
        {
           number_of_bits = Integer.parseInt(args[0]);
           expected_total_element = Integer.parseInt(args[1]);
      ...
      */
        ...
        options.addOption("bf","bloom-filter",false,"use cache to store data");
        options.addOption("bfargs","bloom-filter arguments",true,"BloomFilter Takes 3 arguments(number_of_bits,total_expected_element,hash_num)");
        options.addOption("bfile","bloomfilter input file",true,"takes it to check membership");
        options.addOption("cat","category",true,"crteate bloomfilter On  Category Basis");
        // read command line options
        CommandLineParser parser = new PosixParser();
        CommandLine cmdline = parser.parse(options, args);

        if(cmdline.hasOption("bfargs"))
        {
          String bloomArgs = cmdline.getOptionValue("bfargs");
          String[] bloomArgsList = bloomArgs.split(",");        
          number_of_bits = Integer.parseInt(bloomArgsList[0]);
          expected_total_element =  Integer.parseInt(bloomArgsList[1]);
          hash_num  = Integer.parseInt(bloomArgsList[2]);  
        }
        boolean bfinput_file = false;
        if(cmdline.hasOption("bfile")){
            bfinput_file = true;    
            file_Name = cmdline.getOptionValue("bfile");
              
        } 
        boolean inputcat = false;
        if(cmdline.hasOption("cat"))
        {
          inputcat = true;
          category = cmdline.getOptionValue("cat");
        }else if(bfinput_file)
         {
            System.out.println("ERROR: cat in compulsory when bfile is given");
         }  
        ...
        if(cmdline.hasOption("bf")){
           bloomfilter = true;
        }else if(cmdline.hasOption("bloom-filter")){
           bloomfilter = true;
        }
        ...
        else if(bloomfilter) //if cmdline argument has option -bf
        {
          if(bfinput_file && inputcat)  //if cmdline arg has option -bfile and -cat
          {
             storage = new MultiBloomFilterCacheStorage(number_of_bits,expected_total_element,hash_num,category,file_Name); 
          }
          else if(inputcat) //if cmdline arg has  option -cat
          {
            storage = new MultiBloomFilterCacheStorage(number_of_bits,expected_total_element,hash_num,category);
          }
          else if(bfinput_file) //if cmdline arg has option -bfile and There is Only One file and NO -cat option
          {
            storage = new BloomFilterStorageCacheStorage(number_of_bits,expected_total_element,hash_num,file_Name);  
          }
          else  //if cmdline argument has only -bf option
            storage = new BloomFilterStorageCacheStorage(number_of_bits,expected_total_element,hash_num); 
       }
        else if (memoryMapped) 
        {
