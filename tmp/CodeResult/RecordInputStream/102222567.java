...
/**
 * This is the core of the Jessie SSL implementation; it implements the
 * {@link javax.net.ssl.SSLSocket} for normal and "wrapped" sockets, and handles
 * all protocols implemented by this library.
 */
public final class SSLSocket extends javax.net.ssl.SSLSocket {
    ...
    //
    // Each part is preceded by a form feed.
    ...
    static {
        ...
        // Add cipher suites for OMA Server
        ...
        // Add cipher suites Gemalto NULL
    ...
    public SSLSocket(Socket socket, String host, int port, boolean autoClose)
            throws IOException {
    ...
    public SSLSocket(Socket socket, SocketChannel channel) throws IOException {
        underlyingSocket = socket;
    ...
    public SSLSocket(InetAddress addr, int port) throws IOException {
        super(addr, port);
        initialize();
    ...
    public SSLSocket(InetAddress addr, int port, InetAddress laddr, int lport)
            throws IOException {
    ...
    public SSLSocket(String host, int port, InetAddress laddr, int lport)
            throws IOException {
    ...
    public String[] getEnabledProtocols() {
        synchronized (session.enabledProtocols) {
            try {
                return (String[]) Util.transform(
                        session.enabledProtocols.toArray(), String.class,
                        "toString", null);
            } catch (Exception x) {
    ...
    public void setEnabledProtocols(String[] protocols) {
        ...
        for (int i = 0; i < protocols.length; i++) {
            if (!(protocols[i].equalsIgnoreCase("SSLv3")
                    || protocols[i].equalsIgnoreCase("TLSv1") || protocols[i]
                    .equalsIgnoreCase("TLSv1.1"))) {
    ...
    public String[] getEnabledCipherSuites() {
        synchronized (session.enabledSuites) {
            try {
                return (String[]) Util.transform(
                        session.enabledSuites.toArray(), String.class,
                        "toString", null);
            } catch (Exception x) {
                RuntimeException re = new RuntimeException(x.getMessage());
                re.initCause(x);
    ...
    public String[] getSupportedCipherSuites() {
        return (String[]) CipherSuite.availableSuiteNames().toArray(
                new String[52]);
    }
    ...
    public synchronized void startHandshake() throws IOException {
        ...
        if (handshakeDone) {
            if (clientMode) {
                ...
                Handshake req = new Handshake(Handshake.Type.HELLO_REQUEST,
                        null);
                req.write(handshakeOut, session.protocol);
                handshakeOut.flush();
                // recordOutput.setHandshakeAvail(req.write(handshakeOut,
                // session.protocol));
            }
            return;
        }
    ...
    public synchronized void close() throws IOException {
        ...
        Alert close = new Alert(Alert.Level.WARNING,
    ...
    public void setEnabledCipherSuites(List suites) {
    ...
    void sendAlert(Alert alert) throws IOException {
        RecordOutputStream out = new RecordOutputStream(socketOut,
                ContentType.ALERT, session.params);
    ...
    private static final byte[] SENDER_CLIENT = new byte[]{0x43, 0x4C, 0x4E,
            0x54};
    private static final byte[] SENDER_SERVER = new byte[]{0x53, 0x52, 0x56,
            0x52};

    private void changeCipherSpec() throws IOException {
        RecordOutputStream out = new RecordOutputStream(socketOut,
                ContentType.CHANGE_CIPHER_SPEC, session.params);
        out.write(1);
    }

    private void readChangeCipherSpec() throws IOException {
        RecordInputStream in = new RecordInputStream(recordInput,
    ...
    private synchronized void setupIO() throws IOException {
        ...
        applicationIn = new SSLSocketInputStream(new RecordInputStream(
                recordInput, ContentType.APPLICATION_DATA), this);
        applicationOut = new SSLSocketOutputStream(new RecordOutputStream(
                socketOut, ContentType.APPLICATION_DATA, session.params), this);
        handshakeIn = new SSLSocketInputStream(new RecordInputStream(
                recordInput, ContentType.HANDSHAKE), this, false);
        handshakeOut = new BufferedOutputStream(new SSLSocketOutputStream(
                new RecordOutputStream(socketOut, ContentType.HANDSHAKE,
                        session.params), this, false), 8096);
    }

    private void handshakeCompleted() {
        handshakeDone = true;
        HandshakeCompletedEvent event = new HandshakeCompletedEvent(this,
                session);
        for (Iterator it = handshakeListeners.iterator(); it.hasNext(); ) {
            try {
                ((HandshakeCompletedListener) it.next())
    ...
    private void doClientHandshake() throws IOException {
        ...
        for (Iterator i = session.enabledSuites.iterator(); i.hasNext(); ) {
            ...
            if (s.getKeyExchange() == "SRP") {
                ...
                extensions.add(new Extension(Extension.Type.SRP, Util.concat(
                        new byte[]{(byte) b.length}, b)));
        ...
        try {
            int flen = Integer.parseInt(Util
                    .getSecurityProperty("jessie.fragment.length"));
            byte[] ext = new byte[1];
            if (flen == 512)
                ext[0] = 1;
            else if (flen == 1024)
        ...
        Random clientRandom = new Random(Util.unixTime(),
        ...
        ClientHello clientHello = new ClientHello(session.protocol,
                clientRandom, sessionId, session.enabledSuites, comp,
                extensions);
        ...
        if (!session.enabledProtocols.contains(version)) {
            ...
            for (Iterator it = session.enabledProtocols.iterator(); it
                    .hasNext(); ) {
        ...
        // The server's version is either unsupported by us (unlikely) or the
        // user
        ...
        if (serverHello.getExtensions() != null) {
            for (Iterator it = serverHello.getExtensions().iterator(); it
                    .hasNext(); ) {
        ...
        if (newSession) {
            ...
            if (suite.getKeyExchange() == "SRP") {
                ...
                clientKA = KeyAgreementFactory
                        .getPartyAInstance(Registry.SRP_TLS_KA);
                ...
                clientAttributes.put(SRP6KeyAgreement.USER_PASSWORD,
                        userSrpPassword);
                try {
            ...
            if (suite.getSignature() != "anon") {
                ...
                X509Certificate[] peerCerts = serverCertificate
                        .getCertificates();
                try {
            ...
            if (msg.getType() == Handshake.Type.SERVER_KEY_EXCHANGE) {
                ...
                if (suite.getSignature() != "anon") {
                    ...
                    if (suite.getKeyExchange() == "RSA") {
                        ...
                        updateSig(sig,
                                ((RSAPublicKey) serverKex).getPublicExponent());
                    } else if (suite.getKeyExchange() == "DHE") {
                        updateSig(sig, ((DHPublicKey) serverKex).getParams()
                                .getP());
                        updateSig(sig, ((DHPublicKey) serverKex).getParams()
                                .getG());
            ...
            if (certReq != null) {
                String alias = session.keyManager.chooseClientAlias(
                        certReq.getTypeStrings(), certReq.getAuthorities(),
                        null);
                if (alias == null && version == ProtocolVersion.SSL_3) {
                    Alert alert = new Alert(Alert.Level.WARNING,
                            Alert.Description.NO_CERTIFICATE);
                    sendAlert(alert);
                } else {
                    X509Certificate[] chain = session.keyManager
                    ...
                    if (DEBUG_HANDSHAKE_LAYER)
                        debug.println(msg);
                    msg.write(dout, version);
                    // recordOutput.setHandshakeAvail(msg.write(dout,
                    // version));;
                    dout.flush();
                    if (chain.length > 0) {
            ...
            if (suite.getKeyExchange() == "RSA") {
                ProtocolVersion v = (ProtocolVersion) session.enabledProtocols
                        .last();
                preMasterSecret = Util.concat(v.getEncoded(), new byte[46]);
                try {
                ...
                EME_PKCS1_V1_5 pkcs1 = EME_PKCS1_V1_5
                        .getInstance((RSAPublicKey) serverKex);
                BigInteger bi = new BigInteger(1, pkcs1.encode(preMasterSecret,
                ...
                if (clientKeys == null
                    ...
                    GnuDHPrivateKey tmpKey = new GnuDHPrivateKey(null,
                            ((DHPublicKey) serverKex).getParams().getP(),
                            ((DHPublicKey) serverKex).getParams().getG(), null);
                    clientKA = KeyAgreementFactory
                            .getPartyBInstance(Registry.DH_KA);
                    ...
                    clientKA = KeyAgreementFactory
                            .getPartyBInstance(Registry.ELGAMAL_KA);
                    Map attr = new HashMap();
                    attr.put(
            ...
            } else if (suite.getKeyExchange() == "PSK") {

                String identityStr = Security
                        .getProperty("jessie.psk.identity");
                byte[] identity = identityStr.getBytes();

                ckex = new ClientKeyExchange(identity);

                preMasterSecret = initPSKPreMasterSecret(identityStr);
            ...
            if (version == ProtocolVersion.SSL_3) {
                ...
                attr.put(TLSRandom.SEED, Util.concat(
                        ("master secret").getBytes("UTF-8"),
                        Util.concat(clientRandom.getEncoded(),
                                serverRandom.getEncoded())));
                genSecret.init(attr);
            }
            ...
            if (certReq != null && clientKeys != null) {
                ...
                try {
                    if (key instanceof DSAPrivateKey) {
                        sig = DSSSignature.sign((DSAPrivateKey) key,
                                vSHA.digest(), CSPRNG.SYSTEM_RANDOM);
                        sigAlg = "DSS";
                    } else if (key instanceof RSAPrivateKey) {
                        SSLRSASignature rsa = new SSLRSASignature(vMD5, vSHA);
                        rsa.setupSign(Collections.singletonMap(
                                ISignature.SIGNER_KEY, key));
                        sig = rsa.sign();
                        sigAlg = "RSA";
                    } else {
        ...
        try {
            ...
            } else // JCESecurityParameters
            {
                ...
                ((Mac) writeMac)
                        .init(new SecretKeySpec(keys[0], suite.getMac()));
                ((Mac) readMac)
                        .init(new SecretKeySpec(keys[1], suite.getMac()));
        ...
        // These indicate a configuration error with the JCA.
            ...
            internalError();
            SSLException x = new SSLException("suite " + suite
        ...
        if (newSession) {
            changeCipherSpec();
            session.params
                    .setDeflating(serverHello.getCompressionMethod() == CompressionMethod.ZLIB);
        ...
        synchronized (session.params) {
            ...
            session.params
                    .setInflating(serverHello.getCompressionMethod() == CompressionMethod.ZLIB);
            session.params.setInMac(readMac);
            session.params.setInCipher(readCipher);
            session.params.notifyAll();
        }

        Finished verify = generateFinished(version,
                (IMessageDigest) md5.clone(), (IMessageDigest) sha.clone(),
                false);
    ...
    private void doServerHandshake() throws IOException {
        ...
        ProtocolVersion server = (ProtocolVersion) session.enabledProtocols
                .last();
        CompressionMethod comp;
        if (clientHello.getCompressionMethods()
                .contains(CompressionMethod.ZLIB))
        ...
        if (clientHello.getExtensions() != null) {
            for (Iterator it = clientHello.getExtensions().iterator(); it
                    .hasNext(); ) {
                ...
                if (ex.getType() == Extension.Type.SERVER_NAME) {
                    ...
                    byte[] b = ex.getValue();
                    remoteUser = new String(ex.getValue(), 1, b[0] & 0xFF,
                            "UTF-8");
        ...
        if (suite.getKeyExchange() == "SRP") {
            ...
            // invalid) master secret, and let the handshake fail at the
            // Finished
            // message. This is better than letting the connecting side know
            // that
            ...
            serverKA = KeyAgreementFactory
                    .getPartyBInstance(Registry.SRP_TLS_KA);
        ...
        if (sessionContext.containsSessionID(new Session.ID(clientHello
                .getSessionId()))) {
            Session old = session;
            session = (Session) sessionContext.getSession(clientHello
                    .getSessionId());
            ...
            if (session.getPeerHost().equals(remoteHost)
                ...
                if (DEBUG_HANDSHAKE_LAYER) {
                    ...
                    debug.println("hosts equal? "
                            + session.getPeerHost().equals(remoteHost));
                    debug.println("same suites? "
                            + old.enabledProtocols.contains(session.protocol));
                }
                session = old;
                session.peerHost = remoteHost;
            ...
            debug.println("rejected session");
            debug.println("have session id? "
                    + sessionContext.containsSessionID(new Session.ID(
        ...
        if (newSession) {
            ...
            if (suite.getSignature() != "anon") {
                ...
                String alias = session.keyManager.chooseServerAlias(
                        suite.getAuthType(), null, null);
            ...
            // export RSA, and SRP) we send a signed public key to be used for
            // the
            // key exchange.
            ...
            // message.
            if ((suite.getKeyExchange() == "RSA" && suite.isExportable() && ((RSAPrivateKey) serverKey)
                    .getModulus().bitLength() > 512)) {
                ...
                if (suite.getSignature() != "anon") {
                    ...
                    sig.setupSign(Collections.singletonMap(
                            ISignature.SIGNER_KEY, signPair.getPrivate()));
                    byte[] buf = clientRandom.getEncoded();
                    sig.update(buf, 0, buf.length);
                    buf = serverRandom.getEncoded();
                    sig.update(buf, 0, buf.length);
                ...
                serverKA = KeyAgreementFactory
                        .getPartyAInstance(Registry.DH_KA);
                Map attr = new HashMap();
                ...
                GnuDHPublicKey pubkey = new GnuDHPublicKey(null, servParams
                        .getParams().getP(), servParams.getParams().getG(),
                        serv_y);
                Signature s = null;
                if (suite.getSignature() != "anon") {
                    ISignature sig = null;
            ...
            if (wantClientAuth || needClientAuth) {
                ...
                CertificateRequest.ClientType[] types = new CertificateRequest.ClientType[]{
                ...
                try {
                    auths = (Principal[]) Util.transform(
                            session.trustManager.getAcceptedIssuers(),
            ...
            if (msg.getType() == Handshake.Type.CERTIFICATE) {
                ...
                clientCanSign = ((clientKey instanceof DSAPublicKey) || (clientKey instanceof RSAPublicKey));
                if (suite.getKeyExchange().startsWith("DH")) {
            ...
            if (suite.getKeyExchange() == "RSA") {
                ...
                try {
                    ...
                    EME_PKCS1_V1_5 pkcs1 = EME_PKCS1_V1_5
                            .getInstance((RSAPrivateKey) kexPair.getPrivate());
                    preMasterSecret = pkcs1.decode(Util.concat(new byte[1],
                            bi.toByteArray()));
                    // rsa.init(kexPair);
                    ...
                    // fails.
                    preMasterSecret = Util.concat(version.getEncoded(),
                            new byte[46]);
                    try {
                        CSPRNG.SYSTEM_RANDOM.nextBytes(preMasterSecret,
                                preMasterSecret.length - 46, 46);
                ...
                byte[] identity = (byte[]) ckex.getExchangeObject();

                String identityStr = Util.toAsciiString(identity);
            ...
            if (clientCanSign && (wantClientAuth || needClientAuth)) {
                ...
                if (clientChain != null && clientChain.length > 0) {
                    ...
                    if (clientKey instanceof RSAPublicKey) {
                        ...
                        sig.setupVerify(Collections.singletonMap(
                                ISignature.VERIFIER_KEY, clientKey));
                        ...
                        try {
                            if (!DSSSignature.verify((DSAPublicKey) clientKey,
                                    cvSHA.digest(),
                                    (BigInteger[]) verify.getSigValue())) {
                                throw new Exception(
                                        "client's certificate could not be verified");
                            }
                        } catch (Exception x) {
                            handshakeFailure();
        ...
        // These indicate a configuration error with the JCA.
            ...
            session.enabledSuites.remove(suite);
            internalError();
            SSLException e = new SSLException("suite " + suite
                    + " not available in this configuration");
            e.initCause(nsae);
            throw e;
        } catch (NoSuchPaddingException nspe) {
    ...
    private byte[][] generateKeys(byte[] server, byte[] client,
                                  ProtocolVersion activeVersion) throws LimitReachedException,
            IOException {
        ...
        if (activeVersion == ProtocolVersion.SSL_3) {
            ...
            attr.put(
                    TLSRandom.SEED,
                    Util.concat("key expansion".getBytes("UTF-8"),
        ...
        if (suite.isExportable()) {
            ...
            if (activeVersion == ProtocolVersion.SSL_3) {
                ...
                if (!suite.isStreamCipher()) {
                    ...
                    attr.put(
                            TLSRandom.SEED,
                            Util.concat("IV block".getBytes("UTF-8"),
                                    Util.concat(client, server)));
                    prf.init(attr);
     ...
     */
    private Finished generateFinished(ProtocolVersion version,
                                      IMessageDigest md5, IMessageDigest sha, boolean client) {
        if (version == ProtocolVersion.SSL_3) {
            if (client) {
            ...
            try {
                seed = Util
                        .concat(label.getBytes("UTF-8"), Util.concat(h1, h2));
    ...
    /**
     * Send a fatal decrypt_error alert.
     */
    ...
    private Alert unknownPskIdentity() throws IOException {
        Alert alert = new Alert(Alert.Level.FATAL,
                Alert.Description.UNKNOWN_PSK_IDENTITY);
        sendAlert(alert);
        fatal();
        return alert;
    }
    ...
    private void throwPeerUnverified(X509Certificate[] chain)
            throws IOException {
    ...
    /**
     * Grab the first suite that is both in the client's requested suites and in
     * our enabled suites, and for which we have the proper credentials.
     *
    ...
    private CipherSuite selectSuite(List suites, ProtocolVersion version)
            throws IOException {
        if (DEBUG_HANDSHAKE_LAYER)
            debug.println("selectSuite req=" + suites + " avail="
                    + session.enabledSuites);
        ...
        for (Iterator i = suites.iterator(); i.hasNext(); ) {
            ...
            for (Iterator j = session.enabledSuites.iterator(); j.hasNext(); ) {
                ...
                if (mySuite.getSignature() != "anon"
                        && session.keyManager != null
                        && session.keyManager.chooseServerAlias(
                        mySuite.getAuthType(), null, null) == null) {
                    if (DEBUG_HANDSHAKE_LAYER)
    ...
    private String askUserName(String remoteHost) {
        ...
        try {
            Class c = Class.forName(Util
                    .getSecurityProperty("jessie.srp.user.handler"));
            handler = (CallbackHandler) c.newInstance();
        } catch (Exception x) {
        }
        TextInputCallback user = new TextInputCallback("User name for "
                + remoteHost + ": ", Util.getProperty("user.name"));
        try {
    ...
    private String askPassword(String user) {
        ...
        try {
            Class c = Class.forName(Util
                    .getSecurityProperty("jessie.srp.password.handler"));
            handler = (CallbackHandler) c.newInstance();
        } catch (Exception x) {
        }
        PasswordCallback passwd = new PasswordCallback(user + "'s password: ",
                false);
        try {
    ...
    private boolean checkCertificates(X509Certificate[] chain) {
        ...
        try {
            Class c = Class.forName(Util
                    .getSecurityProperty("jessie.certificate.handler"));
            handler = (CallbackHandler) c.newInstance();
        } catch (Exception x) {
        }
        ...
        ConfirmationCallback confirm = new ConfirmationCallback(
                "The server's certificate could not be verified. There is no proof"
                        ...
                        + nl + "is valid. Do you wish to continue connecting?",
    ...
    /**
     * Update a signature object with a BigInteger, trimming the leading "00"
     * octet if present.
     *
    ...
    /**
     * Intialise PSK premaster secret based on the given PSK identity.
     *
     * @param pskIdentity The given PSK identity.
     * @return PSK premaster secret
     * @throws FileNotFoundException
     * @throws IOException
     */
    private byte[] initPSKPreMasterSecret(String pskIdentity)
        ...
        // load PSK storage
        Properties pskStorage = new Properties();
        pskStorage.load(new FileInputStream(new File(Security
                .getProperty("jessie.psk.storage.file"))));

        if (pskStorage.containsKey(pskIdentity)) {
            psk = Util.toByteArray(pskStorage.getProperty(pskIdentity));
        }
        ...
        preMasterSecret = new byte[2 * psk.length + 4];
        preMasterSecret[0] = (byte) (psk.length >>> 8 & 0xFF);
        preMasterSecret[1] = (byte) (psk.length & 0xFF);
        preMasterSecret[psk.length + 2] = preMasterSecret[0];
        preMasterSecret[psk.length + 3] = preMasterSecret[1];
        System.arraycopy(psk, 0, preMasterSecret, psk.length + 4, psk.length);
