...
public class TeammatesServlet extends HttpServlet {
	...
	private static final String	OPERATION_COORDINATOR_GETTEAMSUBMISSIONLIST				= "coordinator_getteamsumissionlist";
	...
	private void administratorCleanUp() {
		...
		try {
			courses.cleanUpCourse(courseID);
		}
	...
	private void coordinatorGetSubmissionList() throws IOException {
		...
		if(submissionList.get(0).getPointsBumpRatio() == null) {
			...
			for(Submission s : submissionList) {
				student = null;
				fromList = new ArrayList<Submission>();
				// search for student
				...
				student = courses.getStudentWithEmail(courseID,
						s.getToStudent());
				if(student == null) {
				...
				pointsBumpRatio = evaluations.calculatePointsBumpRatio(
						courseID, evaluationName, s.getFromStudent(), fromList);
				...
				s.setFromStudentName(fromStudentName);
				s.setToStudentName(toStudentName);
			}
		...
		resp.getWriter().write(
				"<submissions>"
						+ parseSubmissionDetailsForCoordinatorListToXML(
								submissionList).toString() + "</submissions>");
	...
	private void studentGetSubmissionResultsList() throws IOException {
		...
		Collections.shuffle(submissionResultsList);
