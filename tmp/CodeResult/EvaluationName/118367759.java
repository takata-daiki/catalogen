...
public class Emails {
	...
	public void informStudentsOfEvaluationChanges(String email,
		...
		try {
			...
			message.setSubject(String.format(HEADER_EVALUATION_CHANGE, courseID,
				evaluationName));
			message.setText("Dear "
				...
				+ "\n You can access the evaluation here: " + Config.inst().TEAMMATES_APP_URL
	...
	/**
	 * Sends an email to a Student informing him of the publishing of results for
	...
	public void informStudentsOfPublishedEvaluation(String email,
		...
		try {
			...
			message.setFrom(new InternetAddress(from));
			message.setSubject(String.format(HEADER_EVALUATION_PUBLISH, courseID,
				evaluationName));
			message.setText("Dear " + studentName + ",\n\n"
				+ "The results of the evaluation: \n\n" + courseID + " "
				+ evaluationName + "\n\n" + "have been published.\n"
				+ "You can view the result here: " + Config.inst().TEAMMATES_APP_URL
				+ TEAMMATES_APP_SIGNATURE);
	 ...
	 *          the evaluation deadline (Precondition: Must not be null)
	 */
	public void remindStudent(String email, String studentName, String courseID,
		String evaluationName, String deadline) {
		try {
			Session session = Session.getDefaultInstance(props, null);
			MimeMessage message = new MimeMessage(session);
			...
			message.setFrom(new InternetAddress(from));
			message.setSubject(String.format(HEADER_EVALUATION_REMINDER, courseID,
				evaluationName));
			message.setText("Dear " + studentName + ",\n\n"
	 ...
	 * Peer-Evaluation System for peer-evaluations. [Coordinator name] has invited
	 * you to use the system to evaluate your team members. These are the steps to
	 * follow. Login to the system: Go to URL {provide the correct url here} Login
	 * as a ‘Student’ using your Google ID. If you do not have a Google ID,
	 * please create one. Join the course: Enter this key : Key
	 * 
	 * 
	 ...
	 * submit.
	 * 
	 * Please inform [coordinator email] if your encounter any problems or if your
	 * team details are not correct.
	 * 
	 * @param email
	...
	public void mailStressTesting(String email, int size) {
		try {
			...
			for (int i = 0; i < size; i++) {
				...
				message.setFrom(new InternetAddress(from));
				message.setSubject("Teammates Mail Stree Testing [" + i + "|" + size
					+ "]");
