...
public class SiteAction extends PagedResourceActionII {
	...
	private static final String STATE_SKIP_MANUAL_SELECTION = "skip_manual_selection";
	...
	private String buildContextForTemplate(String preIndex, int index, VelocityPortlet portlet,
		...
		case 0:
			...
			for (int sTypeIndex = 0; sTypeIndex < sTypes.size(); sTypeIndex++) {
				...
				String typeName = SiteService.getLocalizedSiteType(type);
				views.put(type, rb.getFormattedMessage("java.sites", new Object[]{typeName}));
			}
			...
			if (!moreTypes.isEmpty())
			{
				...
				{
					String typeName = SiteService.getLocalizedSiteType(mType);
					views.put(mType, rb.getFormattedMessage("java.sites", new Object[]{typeName}));
				}
			}
			// default view
		...
		case 1:
			 ...
			 * 
			 */
			List<String> types = (List) state.getAttribute(STATE_SITE_TYPES);
			List<String> mTypes = siteTypeProvider.getTypesForSiteCreation();
			Map<String, String> localizedSiteTypes = new HashMap<String, String>();
			if (mTypes != null && !mTypes.isEmpty())
			{
				types.addAll(mTypes);
			}
			for (String type: types) {
				localizedSiteTypes.put(type, SiteService.getLocalizedSiteType(type));
			}
			context.put("siteTypes", types);
			context.put("localizedSiteTypes", localizedSiteTypes);
		...
		case 12:
			...
			try {
				...
				if (allowUpdateSite || allowViewRoster
					...
					sortedAsc = (String) state.getAttribute(SORTED_ASC);
					if (sortedBy == null) {
						if (ServerConfigurationService.getBoolean("default.sortUserByEid", false)) {
							state.setAttribute(SORTED_BY, SiteConstants.SORTED_BY_PARTICIPANT_EID);
							sortedBy = SiteConstants.SORTED_BY_PARTICIPANT_EID;
						} else {
		...
		case 36:
			...
			context.put("skipManualSelection", state.getAttribute(STATE_SKIP_MANUAL_SELECTION));
	...
	private boolean putSelectedProviderCourseIntoContext(Context context, SessionState state) {
		...
		if (state.getAttribute(STATE_ADD_CLASS_PROVIDER_CHOSEN) != null) {
			...
			Map<String, String> providerSectionListTitles = new TreeMap<String, String>();
	...
	private boolean coursesIntoContext(SessionState state, Context context,
		...
		if (providerCourseList != null && providerCourseList.size() > 0) {
			...
			Map<String, String> sectionTitles = new TreeMap<String, String>();
	...
	private void redirectCourseCreation(ParameterParser params, SessionState state, String termFieldName) {
		...
		if (academicSessionEid != null)
		{
			...
			if (t != null) {
				...
				boolean skipManualSelection = ServerConfigurationService.getBoolean("wsetup.skipManualSelection", false);
				if (sections != null && sections.size() > 0 || skipManualSelection) {
					state.setAttribute(STATE_SKIP_MANUAL_SELECTION, skipManualSelection);
					state.setAttribute(STATE_TERM_COURSE_LIST, sections);
					state.setAttribute(STATE_TEMPLATE_INDEX, "36");
