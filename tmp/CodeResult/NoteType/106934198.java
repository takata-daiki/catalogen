/**
 * Archivists' Toolkit(TM) Copyright ďż˝ 2005-2007 Regents of the University of California, New York University, & Five Colleges, Inc.
 * All rights reserved.
 *
...
package
		...
		edu.byu.plugins.importExport.eadExport;
...
public class BYU_EADExport {
    ...
    private void setDidChildren(ResourcesCommon resource, List didChildren) {
		...
		Physdesc physdesc;
		List physdescChildren;
		Extent extent;
		List extentChildren;
		String extentString;
		Physfacet physicalFacet;
		Dimensions dimensions;
		for (ArchDescriptionPhysicalDescriptions thisPhysicalDescription: resource.getPhysicalDesctiptions()) {
			physdesc = new Physdesc();

			physdescChildren = physdesc.getContent();

			extent = new Extent();
			extentChildren = extent.getContent();
			extentString = "";
			if (thisPhysicalDescription.getExtentNumber() != null &&
				thisPhysicalDescription.getExtentNumber().toString().length() > 0) {
				try {
					extentString =
							thisPhysicalDescription.getExtentDisplay();
			...
			extent = new Extent();
			if (thisPhysicalDescription.getContainerSummary() != null && thisPhysicalDescription.getContainerSummary().length() > 0) {
				extent.getContent().add(thisPhysicalDescription.getContainerSummary());
				JAXBElement je = ob.createPhysdescExtent(extent);
				physdesc.getContent().add(je);
			}
			if (thisPhysicalDescription.getPhysicalDetail() != null && thisPhysicalDescription.getPhysicalDetail().length() > 0) {
				physicalFacet = new Physfacet();
				physicalFacet.getContent().add(thisPhysicalDescription.getPhysicalDetail());
				physdesc.getContent().add(ob.createPhysdescPhysfacet(physicalFacet));
			}
			if (thisPhysicalDescription.getDimensions() != null && thisPhysicalDescription.getDimensions().length() > 0) {
				dimensions = new Dimensions();
				dimensions.getContent().add(thisPhysicalDescription.getDimensions());
				physdesc.getContent().add(ob.createPhysdescDimensions(dimensions));
			}
		...
		for (ArchDescriptionDates thisDate: resource.getArchDescriptionDates()) {
			Unitdate unitdate = new Unitdate();
			String bulkDateBegin = thisDate.getIsoBulkDateBegin();
			String bulkDateEnd = thisDate.getIsoBulkDateEnd();


			if (bulkDateBegin.length() > 0 && bulkDateEnd.length() > 0) {
				unitdate.setNormal(bulkDateBegin + "/" + bulkDateEnd);
				unitdate.setType("bulk");
				setUnitDateAttributes(unitdate, thisDate.getEra(), thisDate.getCalendar(), thisDate.getCertainty());
				if (bulkDateBegin.equals(bulkDateEnd))
					unitdate.getContent().add("bulk " + convertISODateForOutput(bulkDateBegin));
				else
					unitdate.getContent().add("bulk " + convertISODateForOutput(bulkDateBegin) + "-" + convertISODateForOutput(bulkDateEnd));
				didChildren.add(unitdate);
			}


			Unitdate unitdate2 = new Unitdate();
			String dateBegin = thisDate.getIsoDateBegin();
			String dateEnd = thisDate.getIsoDateEnd();

			setUnitDateAttributes(unitdate2, thisDate.getEra(), thisDate.getCalendar(), thisDate.getCertainty());

			if (dateBegin != null && dateBegin.length() > 0 && dateEnd != null && dateEnd.length() > 0) {
				unitdate2.setNormal(dateBegin + "/" +dateEnd);

				if (!dateBegin.equals(dateEnd)) {
					unitdate2.setType("inclusive");
				}

				System.out.println("date begin: " + dateBegin + " date end: " + dateEnd + " date expression: " + thisDate.getDateExpression() + "xxxx");
				if (thisDate.getDateExpression() == null || thisDate.getDateExpression().length() == 0) {

					System.out.println("date begin: " + dateBegin + " date end: " + dateEnd);
					if (!dateBegin.equals(dateEnd)) {
						System.out.println("date begin: " + dateBegin + " date end: " + dateEnd);
						unitdate2.getContent().add(convertISODateForOutput(dateBegin) + "-" + convertISODateForOutput(dateEnd));
					} else {
						System.out.println("date begin: " + dateBegin + " date end: " + dateEnd);
						unitdate2.getContent().add(convertISODateForOutput(dateBegin));
					}

				} else {
					unitdate2.getContent().add(thisDate.getDateExpression());
				}
				...
				if (thisDate.getDateExpression() != null && thisDate.getDateExpression().length() != 0) {
					unitdate2.getContent().add(thisDate.getDateExpression());
					unitdate.setLabel(thisDate.getDateExpression());
					didChildren.add(unitdate2);
				}
			}
		}
	...
	private String convertISODateForOutput(String isoDate) {

		if(isoDate.length() == 4) { //just a year
			return isoDate;
		} else if (isoDate.length() == 7) { //year and month
			return isoDate.substring(0,4) + " " + getMonth(isoDate.substring(5,7));
		} else {
			String dayString = isoDate.substring(8);
			if (dayString.charAt(0) == '0') {
				dayString = isoDate.substring(9);
			}
			return isoDate.substring(0,4) + " " + getMonth(isoDate.substring(5,7)) + " " + dayString;
		}
	}
	...
	private void setUnitDateAttributes(Unitdate unitdate, String era, String calendar, Boolean certainty) {
		if (era != null && era.length() > 0) {
			unitdate.setEra(era);
		}
		if (calendar != null && calendar.length() > 0) {
			unitdate.setCalendar(calendar);
		}
		if (certainty != null && certainty) {
			unitdate.setCertainty("circa");
		}
	}
