...
public class PaymentAllocationWriter extends DownloadWriter implements MediaWritable {
	...
	public MemoryFileBuffer writeXLS(IWContext iwc, Company company, Season season, Period period, IWTimestamp fromDate, IWTimestamp toDate, Map entries) throws Exception {
		...
		if (company != null) {
			...
			Collection companies = getBusiness(iwc).getCompanyBusiness().getActiveAndOpenCompanies();
		...
		if (!showOnlyTotals) {
			...
			while (iterator.hasNext()) {
				...
				while (iter.hasNext()) {
					...
					Address address = business.getUserBusiness().getUsersMainAddress(user);
					PostalCode postal = address != null ? address.getPostalCode() : null;
					...
					cell = row.createCell(iCell++);
					if (postal != null) {
						cell.setCellValue(postal.getPostalAddress());
					}
					else {
						cell.setCellValue("");
					}
	...
	public void createTotalsSheet(IWContext iwc, HSSFWorkbook wb, Company company, Season season, Period period, IWTimestamp fromDate, IWTimestamp toDate, IWTimestamp stamp, boolean showOnlyTotals) throws Exception {
		...
		sheet.setColumnWidth((short) 1, (short) (26 * 256));
		...
		if (courses != null) {
			...
			while (iter.hasNext()) {
				...
				if (currentDivision == null) {
					...
					cell = row.createCell((short) 4);
					cell.setCellFormula("SUM(D" + (currentFirstRow + 1) + ":D" + (cellRow) + ")");
					cell.setCellStyle(totalStyle);
