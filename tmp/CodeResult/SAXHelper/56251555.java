...
public class CPortletAdapter
    ...
    protected static Map portletSessionMap;
    ...
    static {
        ...
        portletSessionMap = Collections.synchronizedMap(new HashMap());
        ...
        try {
            ...
            switch (ev.getEventNumber()) {
                ...
                case PortalEvent.SESSION_DONE:
                    ...
                    if (portletSessionMap.get(uid) != null) {
                        WeakReference portletSessionRef = (WeakReference)portletSessionMap.get(uid);
                        PortletSession portletSession = (PortletSession)portletSessionRef.get();
                        if (portletSession != null) {
                            try {
                                ...
                                //log.error(e, e);
                                log.error(e); // This one doesn't print the stack trace
                            }
                            portletSessionMap.remove(uid);
                        }
                    }
    ...
    protected String getMarkup(String uid) throws PortalException {
        ...
        try {
            ...
            PortletSession portletSession = (PortletSession)wrappedRequest.getAttribute(PortletServlet.SESSION_MONITOR_ATTRIBUTE);
            if (portletSession != null) {
                portletSessionMap.put(uid, new WeakReference(portletSession));
            }
            ...
            //log.error(t, t);
            log.error(t); // This one doesn't print the stack trace
