...
public class GWTCanvasImplIE6 implements GWTCanvasImpl {
	...
	public void cubicCurveTo(double cp1x, double cp1y, double cp2x,
		...
		pathStr.push(PathElement.bezierCurveTo(cp1x, cp1y, cp2x, cp2y, x, y,
				this));
		currentX = x;
		currentY = y;
	}
	...
	public void drawImage(ImageElement img, double sourceX, double sourceY,
		...
		vmlStr.push(String.valueOf((fullWidth - sourceX - sourceWidth)
				/ fullWidth));
	...
	public void drawImage(Element img, double sourceX, double sourceY,
			double sourceWidth, double sourceHeight, double destX,
			double destY, double destWidth, double destHeight) {
		// XXX Not implemented in IE6
	}
	...
	public void fill() {
		...
		if (context.fillGradient != null
			...
			shapeStr.push(colorStops.get(colorStops.size() - 1).color
					.toString());
			shapeStr.push("\" type=\"");
	...
	public int getCoordX(double[] matrix, double x, double y) {
		int coordX = doubleToFlooredInt(Math.floor(10 * (matrix[0] * x
				+ matrix[1] * y + matrix[2]) - 4.5f));
	...
	public int getCoordY(double[] matrix, double x, double y) {
		int coordY = doubleToFlooredInt(Math.floor(10 * (matrix[3] * x
				+ matrix[4] * y + matrix[5]) - 4.5f));
		// record current point to derive bounding box of current open path.
	...
	public void scale(double x, double y) {
		context.arcScaleX *= x;
		context.arcScaleY *= y;
		// TODO(jaimeyap): We should try to figure out how to support scaling
		// with
		// different proportions in the X and Y axis. For now, supporting simple
		// 1:1
	...
	private Element createParentElement() {
		// TODO(jaimeyap): We should probably refactor the Widget's constructor
		// to
