...
public class HE_Mesh extends HE_MeshStructure {
	...
	public void splitEdge(final HE_Edge edge, final double[] f) {
		...
		for (int i = 0; i < f.length; i++) {
			...
			if ((fi > 0) && (fi < 1)) {
				v = new HE_Vertex(WB_XYZ.interpolate(v0, v1, fi));
	...
	public HE_Selection triSplitFace(final Integer key, final WB_Point v) {
	 ...
	 *            key of face
	 */
	public void triangulateFace(final Integer key) {
		triangulateFace(getFaceByKey(key));

	}

	public void triangulateFace(final HE_Face face) {
		if (face.getFaceOrder() > 3) {
			final ArrayList<WB_IndexedTriangle2D> tris = face.triangulate();
			final ArrayList<HE_Vertex> vertices = face.getFaceVertices();
			final ArrayList<HE_Halfedge> unpHalfedges = new ArrayList<HE_Halfedge>();
			for (int i = 0; i < tris.size(); i++) {
				...
				add(he2);
				add(he3);
				unpHalfedges.add(he1);
				unpHalfedges.add(he2);
				unpHalfedges.add(he3);
			}

			HE_Halfedge he = face.getHalfedge();
			do {
				he.getPair().clearEdge();
				unpHalfedges.add(he.getPair());
				he.getPair().clearPair();
				remove(he.getEdge());
				remove(he);
				he = he.getNextInFace();
	 ...
	 * @return true or false
	 */
	public boolean contains(final WB_Point p, final boolean isConvex) {
		final WB_Point dir = new WB_Point(Math.random() - 0.5,
		...
		while (fItr.hasNext()) {
			...
			if (isConvex) {
				...
				if (lpi.intersection) {
					if (pointIsInFace(lpi.p1, face)) {
						if (!HE_Mesh.pointIsStrictlyInFace(lpi.p1, face)) {
	...
	public static boolean pointIsInFace(final WB_Point p, final HE_Face f) {
		return WB_Epsilon.isZeroSq(WB_Distance.sqDistance(p,
				WB_ClosestPoint.closestPoint(p, f.toPolygon())));
	}
	...
	public static boolean pointIsStrictlyInFace(final WB_Point p,
		...
		if (!WB_Epsilon.isZeroSq(WB_Distance.sqDistance(p,
				WB_ClosestPoint.closestPoint(p, tris)))) {
			return false;
		}
		if (WB_Epsilon.isZeroSq(WB_Distance.sqDistance(p,
				WB_ClosestPoint.closestPointOnPeriphery(p, poly, tris)))) {
			return false;
		}
		return true;
	}
	...
	public HE_Face fuseCoplanarFace(final HE_Face face, final double a) {
		...
		final ArrayList<HE_Halfedge> halfedges = sel.getOuterHalfedgesInside();
	...
	public void fuseCoplanarFaces(final double a) {
		final ArrayList<HE_Face> faces = this.getFacesAsArrayList();
	 ...
	 * @return WB_KDTree
	 */
	public WB_KDTree<Integer> getFaceTree() {
	 ...
	 * @return WB_KDTree
	 */
	public WB_KDTree<Integer> getVertexTree() {
		final WB_KDTree<Integer> tree = new WB_KDTree<Integer>();
		HE_Vertex v;
		final Iterator<HE_Vertex> vItr = vItr();
		while (vItr.hasNext()) {
	...
	public HE_Vertex getClosestVertex(final WB_Point p,
			final WB_KDTree<Integer> vertexTree) {
	...
	public WB_Point getClosestPoint(final WB_Point p,
		...
		final ArrayList<HE_Face> faces = v.getFaceStar();
		double d;
		double dmin = Double.POSITIVE_INFINITY;
		WB_Point result = new WB_Point();
		for (int i = 0; i < faces.size(); i++) {
			final WB_ExplicitPolygon poly = faces.get(i).toPolygon();
			final WB_Point tmp = WB_ClosestPoint.closestPoint(p, poly);
	 ...
	 * @param v2
	 * 
	 * @return shared faces as ArrayList<HE_Face>
	 */
	public ArrayList<HE_Face> getSharedFaces(final HE_Vertex v1,
			final HE_Vertex v2) {
		final ArrayList<HE_Face> result = v1.getFaceStar();
		final ArrayList<HE_Face> compare = v2.getFaceStar();
	...
	public ArrayList<WB_ExplicitPolygon> getBoundaryAsPolygons() {
		final ArrayList<WB_ExplicitPolygon> polygons = new ArrayList<WB_ExplicitPolygon>();
		final ArrayList<HE_Halfedge> halfedges = getBoundaryHalfedges();
		final ArrayList<HE_Halfedge> loop = new ArrayList<HE_Halfedge>();
	...
	public ArrayList<HE_Halfedge> getBoundaryLoopHalfedges() {
	...
	public void resolvePinchPoints() {
		...
		do {
			...
			while (vItr.hasNext()) {
				...
				final ArrayList<HE_Halfedge> vStar = v.getHalfedgeStar();
