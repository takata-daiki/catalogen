...
public class DefaultJMCProjectService implements JMCProjectService {
    ...
    private final Map<Long, PropertySet> configCache = new MapMaker().concurrencyLevel(8).makeComputingMap(new Function<Long, PropertySet>() {
    ...
    public long countJMCIssues(User user) {
        ...
        final Query query = JqlQueryBuilder.newBuilder().where().customField(uuidField.getIdAsLong()).notEqEmpty().buildQuery();
        try {
