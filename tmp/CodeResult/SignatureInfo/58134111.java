...
public class OperationChannelImplTest extends TestCase {
  ...
  private final WaveletOperationContext.Factory contextFactory =
      new BasicWaveletOperationContextFactory(new ParticipantId(USER_ID));
  ...
  private Delta createRandomDelta(int ops) {
  ...
  private Delta createDelta(long waveVersion, WaveletOperation... ops) {
    Delta delta = new Delta(waveVersion);
   ...
   * and signature.
   */
  private Delta createRandomVersionedDelta(int version, int opCount, byte[] hash) {
    Delta delta = createRandomDelta(opCount);
    delta.setVersion(version);
    delta.setSignature(HashedVersion.of(version + opCount, hash));
    return delta;
  }

  /** Returns a copy of an unsigned delta. */
  private Delta copy(Delta original) {
    Delta copy = new Delta(original.getVersion());
    copy.addAll(original);
    return copy;
  }

  /** Returns a signed copy of an unsigned delta. */
  private Delta signedCopy(Delta original) {
    Delta signed = copy(original);
    signed.setSignature(HashedVersion.of(signed.getVersion() + signed.size(), new byte[0]));
    return signed;
  }
   ...
   */
  private void reconnectChannel(long connectVersion, byte[] connectSignature,
      long currentVersion, byte[] currentSignature, Delta expectRetransmission)
      throws ChannelException {
    ...
    for (HashedVersion rcv : reconnectVersions) {
      if (connectVersion == rcv.getVersion() && connectSignature == rcv.getHistoryHash()) {
  ...
  private Delta sendAndCheckRandomOp(OperationChannelImpl opChannel,
      long currentVersion) throws ChannelException {
    WaveletOperation op = createOp();
    Delta delta = createDelta(currentVersion, op);
  ...
  public void testSimpleSendAndAck() throws Exception {
    ...
    sendAndCheckRandomOp(operationChannel, initialVersion);
    listener.clear();
  ...
  public void testSendToInaccessibleChanneFails() throws ChannelException {
    final HashedVersion signatureInfo = HashedVersion.unsigned(0);
    deltaChannel = new MockWaveletDeltaChannel();
    cc = new ConcurrencyControl(ccLogger, signatureInfo);
    operationChannel = new OperationChannelImpl(opLogger, deltaChannel, cc,
    ...
    try {
      sendAndCheckRandomOp(operationChannel, 0);
  ...
  public void testReconnectAfterCommit() throws Exception {
    ...
    sendAndCheckRandomOp(operationChannel, 42);
    ...
    // Receive next delta.
    Delta delta = createRandomVersionedDelta(43, 1, SIG3);
  ...
  public void testReconnectWithPendingAckNotRecievedByServer() throws Exception {
    ...
    Delta delta = sendAndCheckRandomOp(operationChannel, 42);

    // The server's version is still 42 when it responds. Expect a retransmission.
    reconnectChannel(initialVersion, initialSignature, initialVersion, initialSignature, delta);
  ...
  public void testReconnectWithPendingAckAndNewDeltaSentByServer() throws Exception {
    ...
    // Receive another op
    delta = createRandomVersionedDelta(43, 1, SIG4);
    operationChannel.onDelta(delta);
  ...
  public void testReconnectWithPendingAckAndNewCoalescedDeltaSentByServer() throws Exception {
    ...
    Delta merged = copy(delta);
    // Coalesce another op into the message.
    merged.add(createOp());
  ...
  public void testReconnectFailureWithAckAndPendingAckWithCollidingVersionNumber()
    ...
    Delta delta2 = sendAndCheckRandomOp(operationChannel, 43);
  ...
  public void testReconnectWherePendingAckCollisionCanBeRecovered() throws Exception {
    ...
    // Send a delta.
    Delta delta1 = sendAndCheckRandomOp(operationChannel, 43);

    // Reconnect at 43.
    reconnectChannel(43, signature43, 44, SIG3, null);

    // Now receive a different delta to the un-acked delta1.
    Delta delta2 = createRandomVersionedDelta(43, 1, SIG4);
    ...
    Delta delta3 = createDelta(45, op3);
  ...
  public void testReconnectWhereAnAckWasNotCommittedCanBeRecovered() throws Exception {
    ...
    Delta delta1 = sendAndCheckRandomOp(operationChannel, 42);
