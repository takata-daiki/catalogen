...
public class ExcelFamilieBetreuung {
	...
	public void erstelleExcel(ArrayList<String> spalten) {
		try {
			...
			fileOutputStream = new FileOutputStream("Familien Betreuung.xlsx");
			...
			for (Familie bet : familien) {
				ArrayList<BetreuungFamilie> z = bet.getBetreuung();
				if (z != null) {
			...
			for (int i = 0; i < familien.size(); i++) {
				...
				ArrayList<BetreuungFamilie> betreuungen = b.getBetreuung();
				...
				if (betreuungen != null) {
					...
					for (int z = 0; z < countBetreuung; z++) {
						...
						if (z < betreuungen.size()) {
							...
							BetreuungFamilie betreuung = betreuungen.get(z);
